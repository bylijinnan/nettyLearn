 ...
 * 跑完之后产生一个事件（改变state），再通知handler执行IO操作
 * 这使得handler只专注IO操作
 */
public class HandlerWithThreadPool extends Handler {
    ...
    //从SocketChannel中读到数据后，把“数据的处理”这个工作扔到线程池里面执行
    void read() throws IOException {
        int readCount = socketChannel.read(input);
        if (readCount > 0) {
            state = PROCESSING;
            
            //execute是非阻塞的，所以要新增一个state（PROCESSING），表示数据在处理当中，不能执行send操作
            pool.execute(new Processer(readCount)); 
        }
        //We are interested in writing back to the client soon after read processing is done.
        //这时候虽然设置了OP_WRITE，但下一次本handler被选中时不会执行send()方法，因此state=PROCESSING
        //或者可以把这个设置放到Processer里面
